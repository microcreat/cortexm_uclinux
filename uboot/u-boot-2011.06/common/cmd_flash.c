...
int do_flinfo ( cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (argc == 1) {	/* print info for all FLASH banks */
		for (bank=0; bank <CONFIG_SYS_MAX_FLASH_BANKS; ++bank) {
			printf ("\nBank # %ld: ", bank+1);
#ifdef CONFIG_CHIP_FLASH
            if(CONFIG_CHIP_FLASH_ID == bank)
			    chip_flash_print_info (&flash_info[bank]);
			else
#endif
	...
	printf ("\nBank # %ld: ", bank);
#ifdef CONFIG_CHIP_FLASH
    if(CONFIG_CHIP_FLASH_ID == (bank-1))
		chip_flash_print_info (&flash_info[bank-1]);
	else
#endif		
		...
		for (bank=0,info = &flash_info[0];
			...
			if (s_first[bank]>=0) {
				...
				debug("Erase Flash from 0x%08lx to 0x%08lx "
					...
					bank+1);
#ifdef CONFIG_CHIP_FLASH
        if(flash_info[CONFIG_CHIP_FLASH_ID].flash_id == info->flash_id)						
				rcode = chip_flash_erase (info, s_first[bank], s_last[bank]);
		else
#endif			
